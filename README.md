...
```bash
make knowledge-search QUERY='提现已经完成但钱包没有到账'
```

`knowledge-index` 和 `knowledge-search` 需要配置 `DASHSCOPE_API_KEY`；`knowledge-search`
必须显式传入 `QUERY`，否则 Makefile 会直接拒绝执行。
...
```bash
make evaluate-conversation-flows
```
