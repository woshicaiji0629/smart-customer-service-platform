...
- category: withdrawal | deposit | identity_verification | account_security |
  spot_trading | general_platform | other
- intent: status_query | missing_arrival | failure_reason | fee_rule |
  onchain_status | rule | memo_tag_issue | verification_failure |
  compromised | two_factor_issue | security_general | trading_question |
  platform_operation | human_only | out_of_scope | unknown
- confidence: 0 到 1 的数字
- entities: 只允许 order_id、txid、coin、network、time_hint、
  verification_type、failure_reason，值必须是字符串
- missing_fields: 当前问题继续自动处理前必须补充的字段名数组

规则：
1. 用户描述了可处理的具体问题时，即使提到人工，也优先识别具体问题，不选 human_request。
2. business_query 当前仅用于查询具体提现订单状态或具体充值 TxID 状态，
   缺少必要字段时放入 missing_fields。
...
9. 提现失败原因、处理规则等通用问题使用 knowledge_rag；
   只有用户在查询自己的具体提现订单状态时才使用 business_query。
